...
def assocCoeff(data,ac,sc,predictor):
   ...
   for x in range(50,100):
     ...
     #    phi = math.sqrt((math.pow(tp*tn - fp*fn,2)/(tp+fp)*(fn+tn)*(tp+fn)*(fp+tn)))
      ...
      acstats[str(x)] = {'tn':tn,'fn':fn,'tp':tp,'fp':fp,'phi':phi,'tpp':tpp,'fpp':fpp,'acc':acc}
   return (acstats)
...
for a,b,c in v:
   ...
   f.write("Cutoff\tTn\tFn\tTp\tFp\tPhi\tTPP\tFPP\tAccuracy\n")
   cutoffs = stats.keys()
   cutoffs.sort()
   for c in cutoffs:
      f.write(str(c)+"\t"+"\t".join([str(stats[c]['tn']),str(stats[c]['fn']),str(stats[c]['tp']),str(stats[c]['fp']),str(stats[c]['phi']),str(stats[c]['tpp']),str(stats[c]['fpp']),str(stats[c]['acc'])])+"\n")
   f.close()
