...
'''Methods for the FACS dose response curve package.
...
import tempfile
import pdb
import os, os.path, sys, StringIO,subprocess
...
# Well translation matrix to convert from horizontal to vertical numbering
###########################################################################
...
def doDRCAnalysis(antigens,ag_names,files,clonelist,layout,conditions,paired_ag,mod_antigens,conc,units,log,fit):
    # antigens:  {u'Lep-': ['Median__ss_293F', 'Median__ss_293F_H8', 'Median__ss_CHOK1', 'Median__ss_CHO_M8'], u'Lep+': ['Median__ss_293F', 'Median__ss_293F_H8', 'Median__ss_CHOK1', 'Median__ss_CHO_M8']}
    # ag_names:  ['Median__ss_293F', 'Median__ss_CHO_M8', 'Median__ss_293F_H8', 'Median__ss_CHOK1']
    # files:  {'/opt/local/var/media/facstool/rawdata_9Wi7NO': u'tranc_130529_40IgGs_Lep_P1.csv', '/opt/local/var/media/facstool/rawdata_A2DYVW': u'tranc_130529_40IgGs_NoLep_P1.csv'}
    # clonelist:  Clone1,Clone2,Clone3,Clone4,Clone5,Clone6,Clone7,Clone8,Clone9,Clone10,Clone11,Clone12
    # layout:  8
    # conditions:  {'/opt/local/var/media/facstool/rawdata_9Wi7NO': u'Lep+', '/opt/local/var/media/facstool/rawdata_A2DYVW': u'Lep-'}
    # paired_ag:  [(u'293F', u'293F_H8'), (u'CHO_M8', u'CHOK1')]
    # mod_antigens:  {'Median__ss_293F': u'293F', 'Median__ss_CHO_M8': u'CHO_M8', 'Median__ss_293F_H8': u'293F_H8', 'Median__ss_CHOK1': u'CHOK1'}
    # conc:  25, 8.33, 2.77, 0.92, 0.30, 0.10, 0.03, 0.01
    # units:  ug
    # log:  yes
    # fit:  L5
    print "**********\nantigens: ",antigens
    print "ag_names: ",ag_names
    print "files: ",files
    print "clonelist: ",clonelist
    print "layout: ",layout
    print "conditions: ",conditions
    print "paired_ag: ",paired_ag
    print "mod_antigens: ",mod_antigens
    print "conc: ",conc
    print "units: ",units
    print "log: ",log
    print "fit: ",fit

    multiple,layout = getPlatePattern(int(layout))
    print "MULTIPLE=",multiple
    conc_range = createConcRange(conc,multiple)
    print "CONC_RANGE: ",conc_range
    datasets,input_file = createDataset(files.keys(),conditions,mod_antigens.values(),clonelist.split(','),multiple,conc_range)
    print "success so far"
   # print input_file.getvalue()
    graphfile, jpg_names = doDRC(input_file,conditions,clonelist.split(','),paired_ag,units)
    return input_file,graphfile, jpg_names
...
def getPlatePattern(layout):
    ...
    if layout == 8: 
        ...
        layout = 'HORIZ'
        multiple = 8
    return multiple,layout


##################################################
# getAgList                                      #
# files = list of file paths for tmp input files #
# conditions = dict of {file paths: conditions}  #
# @rvalue = dict of {condition: [Ags]}           #
##################################################
def getAgList(files,conditions):
    ...
    for fn in files:
        f = open(fn,'r')
        for line in f:
            # replace funky chars that R might not like later
            vals = line.rstrip().replace(' ','_').replace(':','_').replace('/','_').replace('-','_').replace('"','').replace('.','_').split(',') 
            if vals[0]=='Sample' and vals[-2:len(vals)-1][0].find('Ratio') == 0 and vals[-1:len(vals)][0].find('Ratio')==0:
                ...
                f.close()
                break                    
        ag_names.update(antigens[conditions[fn]])
    return antigens,list(ag_names)
...
###############################################################################
# createDataset                                                               #
# Loops through submitted input files and parses out the data values.         #
# Writes a new input file in a different format that combines all data        #
# files and parameters in a format that is R compatible. Also creates a       #
# comprehensive dictionary that holds the data                                #
# files = dict{tmp_filename:conditions}                                       #
# antigens = list of angtigens                                                #
# clonelist = ordered list of clone names                                     #
# @rvalue: filelike object to be returned to user and used by R, dict of data #
###############################################################################
def createDataset(files,conditions,antigens,clonelist,multiple,conc2):
    ...
    with tempfile.NamedTemporaryFile(prefix='processed_data_', dir=MEDIA_ROOT+'/facstool/', delete=False) as tmp:
        tmp.write('Conditions,Clones,Conc,'+','.join(antigens)+'\n')
        print "ANTIGENS ADDED WERE: ",','.join(antigens)
        for fn in files:
            f = open(fn,'r')
        ...
        tmp.close()
        subprocess.call(['chmod', '777', tmp.name])
        ...
        for c in subsets['Conditions']:
            for clone in subsets['Conditions'][c]['clones']:
                for ag1,ag2 in comparisons:
                    ...
                    if d == 'jpg':
                        jname = path+c+"_"+clone+'_'+ag1+'_'+ag2+'_graph.jpg'
                    ...
                    # Draw the graphs
                    plot(p,type='all',ylim=r.c(0,maxvalue),main=c+" ("+ag2+" \nvs.\n "+ag1+") "+clone,lwd=2,col="blue",ylab="MFI", xlab="Conc ("+units+")",log='x')
                    ...
                    if d == 'jpg': grdevices.dev_off()

        if d == 'pdf': grdevices.dev_off()    # close multi-graph file
