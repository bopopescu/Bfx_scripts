...
Trims reads in bam file to the gene start and stop

Created by Mark Evans on 2012-10-12.
Copyright (c) 2012 __Monogram Biosciences__. All rights reserved.

Revised
2012-12-06  Added offset parameter with default of 0. Gilead wanted their data to have +10bp at start and stop of gene seq.
...
##############################################################################################################
def extractRegion(bamfile,offset):
    ...
    gene_pos = {'1b_Con1_full_reference_seq':{'ns5b':{'nterm':7599,'cterm':9371}},
                ...
                'Con1l_NS5B_gene':{'ns5b':{'nterm':200,'cterm':1776}},
    ...
    for read in read_pool:
        ...
        elif read.pos < gene_pos[ref]['ns5b']['nterm']: # and not (read.aend < gene_pos[ref]['ns5b']['nterm']):

            q = gene_pos[ref]['ns5b']['nterm'] - read.pos - 1 - offset
        ...
        elif ((read.pos-read.qstart) + len(read.seq)) > gene_pos[ref]['ns5b']['cterm']: # and read.pos < gene_pos[ref]['ns5b']['cterm']:
...
# processFiles #
################
def processFiles(seqfile,offset):
    if seqfile == 'all':
        path =""
        for infile in glob.glob (os.path.join(path,"*")):
            if infile[-4:] == '.bam':
                extractRegion(infile,offset)
    else:
        extractRegion(seqfile,offset)
...
def main(argv):
    ...
    for opt, arg in opts:
        ...
        elif opt in ("-o","--offset"): offset = int(arg)
   
    if seqfile !='':
        processFiles(seqfile,offset)
    else:
        print opts
        print sys.argv
        usage()
