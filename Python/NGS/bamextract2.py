...
# processFiles #
################
def processFiles(seqfile,start,stop,output,exact):
#    if seqfile == 'all':
#        path =""
...
#    else:
    ...
    extractRegion(seqfile,start,stop,output,exact)
...
###########
def main(argv):
    seqfile = start = stop = output = exact = ''
    ...
    if seqfile !='' and start != '' and stop != '' and output !='':
        processFiles(seqfile,start,stop,output,exact)
    else:
        print opts
        print sys.argv
        usage()
