...
Updated 03.16.2012  Rewrote updateGenoReport, updatePhenoReport and updateSeqMutations to be more accurate in preventing duplicates and more efficient 
Updated 03.15.2012  Rewrote updateSeq method to be more efficient. Previous version took two weeks on Bali to repopulate seq/mut_freq tables
                    from scratch. Current version now takes 20 hours on Bali (244440 records). Also moved the MySQL connection to be first executed
                    within the updateGTRuleResultRows method because if it was created at the beginning with the others, it would be closed by the time
                    the process needed it during a long (20 hour) update.
...
"""
Note:
# We use Get Oracle records > last_update because the 
...
"""
Dev Note:
Can probably replace most insert sql loops with copy_to.  This should result in major speed up
across the board due to significantly less I/O. Have already done this for the two slowest steps
"""
...
def updateSeq(pgc,cxc,LOG):
    ...
    # Get date of last updates from WH
    ###################################
    ...
    if pgr != "Error":
        up_date = ''
        if type(pgr[0][0]) is NoneType :                        # allow for clean reload of all data if table was deleted prior to execution
            sql = update_seq_sql % ('1969-07-20','1969-07-20')  # The Eagle has landed
            up_date = '1969-07-20'
        else:
            sql = update_seq_sql % (pgr[0][0],pgr[0][0])        # last_update 
            up_date = pgr[0][0]
        LOG.write(gt()+"\t\tLast update was "+up_date+"\n")
        
        # Retrieve all new records from Oracle vlps
        ############################################
        cxr = getOracle(cxc,sql,'',LOG)
        if cxr != "Error" and len(cxr) != 0:
			
            # Identify duplicate records if any
            ####################################
            data_ids = []
            for r in cxr:
                data_ids.append((r[0],r[2],r[9])) # data_file_id, vrl_accession, assay_type
            sql = "select distinct seq_id, vrl_accession,assay_type from wh.seq where seq_id in %s and vrl_accession in %s and assay_type in %s"
            r_ids = tuple(x[0] for x in data_ids)
            v_ids = tuple(x[1] for x in data_ids)
            a_ids = tuple(x[2] for x in data_ids)  # only 3 assay types, so this is kinda dumb
            pgresult = getPG(pgc,sql,(r_ids,v_ids,a_ids),LOG)
            ...
            if pgresult !="Error":
                if len(pgresult) !=0:
                    for x,y,z in pgresult:
                        sql = "delete from wh.seq where seq_id=%s and vrl_accession=%s and assay_type=%s"
                        bad_id = "|".join([str(x),y,str(z)])  
                        execPG(pgc,sql,(x,y,z),LOG)						
                        dup_ids.append(bad_id)
            else:
            ...
            for row in cxr:        
                ...
                check_freq='yes'
                if "|".join([str(vals[0]),vals[2],str(vals[9])]) in dup_ids: check_freq = 'no'
...
def updateSeqMutations(pgc,cxc,LOG):
    ...
    if pgr != "Error":
        up_date = ''
        if type(pgr[0][0]) is NoneType :                        # allow for clean reload of all data if table was deleted prior to execution
            sql = update_seq_mutations_sql % '1969-07-20'       # The Eagle has landed
            up_date = '1969-07-20'
        else:
            sql = update_seq_mutations_sql % pgr[0][0]          # last_update 
            up_date = pgr[0][0]
        LOG.write(gt()+"\t\tLast update was "+up_date+"\n")
        ...
        if cxr != "Error":
            ...
            if len(cxr) != 0:
                ...
                # Identify duplicate records if any and remove those for gt_report_ids that have been dropped from seq
                #######################################################################################################
                data_ids = []
                for r in cxr:  data_ids.append(r[0])

                # Step1: remove those previously deleted from seq
                sql = "select distinct a.gt_report_id from seq_mutations a left outer join seq b on a.gt_report_id=b.gt_report_id where b.gt_report_id is null"
                pgresult = getPG(pgc,sql,'',LOG)
                d_ids = tuple(x[0] for x in pgresult)
                print "d_ids: ",str(d_ids),"\n"
                sql = "delete from seq_mutations where gt_report_id in %s"
                execPG(pgc,sql,(d_ids,),LOG)
                LOG.write(gt()+"\t\tThere were "+str(len(d_ids))+" old records removed from the table\n")

                # Step2: Identify any duplicates in incoming records and remove them. Step1 should make this result in 0 removed, but just in case
                ...
                data = ""
                for m in cxr:
                    data = data + "\t".join([str(m[0]),str(m[1]),str(m[2]),m[3],m[4],str(m[5])])+"\n"

                # Write seq_mutation data back to table
                f = StringIO(data)  # create a file-like object
                pgc.copy_from(f,'wh.seq_mutations',columns=('gt_report_id','seq_id','mutation_id','mutation_type','mutation','rep_date'))
...
def updatePSGTResults(pgc,cxc,LOG):
    ...
    if pgr != "Error":
        last_update = ''
        if type(pgr[0][0]) is NoneType :                    # allow for clean reload of all data if table was deleted prior to execution
            sql = update_psgtresult_sql % '1969-07-20'      # The Eagle has landed
            up_date = '1969-07-20'
        else:
            sql = update_psgtresult_sql % pgr[0][0]         # last_update 
            last_update = pgr[0][0]
        LOG.write(gt()+"\t\tLast update was "+last_update+"\n")
        ...
        if cxr != "Error":
            ...
            if len(cxr) != 0:
                ...
                for r in cxr:
                    data_ids.append(r[1])
                sql = "select distinct vrl_accession from wh.psgt_result where vrl_accession in %s"
                r_ids = tuple(x for x in data_ids)
                pgresult = getPG(pgc,sql,(r_ids,),LOG)
                ...
                if pgresult !="Error":
                    if len(pgresult) !=0:
                        for x in pgresult:
                            sql = "delete from wh.psgt_result where vrl_accession=%s" % x[0]
                            execPG(pgc,sql,'',LOG)						
                else:
...
def updateGenoReport(pgc,cxc,LOG):
    ...
    if pgr != "Error":
        last_update = ''
        if type(pgr[0][0]) is NoneType :                    # allow for clean reload of all data if table was deleted prior to execution
            sql = update_geno_report_sql % '1969-07-20'     # The Eagle has landed
            last_update = '1969-07-20'
        else:
            sql = update_geno_report_sql % pgr[0][0]        # last_update 
            last_update = pgr[0][0]
        LOG.write(gt()+"\t\tLast update was "+last_update+"\n")

        cxr = getOracle(cxc,sql,'',LOG)
        dup_ids = []  # use this to capture gt_report_id's that will be dropped, so they can also be dropped from geno_rep_row table
        ...
        if cxr != "Error":
            ...
            if len(cxr) != 0:
                ...
                # Identify duplicate records if any, and remove them from table
                ################################################################
                data_ids = []
                for r in cxr:
                    data_ids.append((r[1],r[7]))       # vrl_accession, assay_type
                sql = "select distinct vrl_accession, assay_type, gt_report_id from wh.geno_report where vrl_accession in %s and assay_type in %s"
                v_ids = tuple(x[0] for x in data_ids)
                a_ids = tuple(x[1] for x in data_ids)  # only 3 assay types, so this is kinda dumb
                pgresult = getPG(pgc,sql,(v_ids,a_ids),LOG)

                if pgresult !="Error":
                    if len(pgresult) !=0:
                        for x,y,z in pgresult:
                            sql = "delete from wh.geno_report where vrl_accession=%s and assay_type=%s"
                            execPG(pgc,sql,(x,y),LOG)    # x,y are used to ensure unique report
                            dup_ids.append(str(z))       # z is used to ensure unique data in geno_rep_row later
                else:
                    LOG.write(gt()+"\twh.geno_report and wh.geno_rep_row updates were skipped, there was a problem checking for existing wh.geno_report records\n")
                    return 'bad'
            ...
            LOG.write(gt()+"\tFailed to retrieve update from Oracle, wh.geno_report and wh.geno_rep_rows were not updated\n")
            return 'bad'
        
        # Update geno_rep_row table
        #############################
...
def updatePhenoReport(pgc,cxc,LOG):
    ...
    if pgr != "Error":
        last_update = ''
        if type(pgr[0][0]) is NoneType :                     # allow for clean reload of all data if table was deleted prior to execution
            sql = update_pheno_report_sql % '1969-07-20'     # The Eagle has landed
            last_update = '1969-07-20'
        else:
            sql = update_pheno_report_sql % pgr[0][0]        # last_update 
            last_update = pgr[0][0]
        LOG.write(gt()+"\t\tLast update was "+last_update+"\n")

        cxr = getOracle(cxc,sql,'',LOG)
        dup_ids = []  # use this to capture ids that are dups and will be dropped, so that they can also be removed from pheno_rep_rows
        ...
        if cxr != "Error":
            ...
            if len(cxr) != 0:
                ...
                data_ids = []
                for r in cxr:
                    data_ids.append((r[1],r[4]))       # vrl_accession, assay_type
                sql = "select distinct vrl_accession, assay_type, pt_report_id from wh.pheno_report where vrl_accession in %s and assay_type in %s"
                v_ids = tuple(x[0] for x in data_ids)
                a_ids = tuple(x[1] for x in data_ids)  # only 3 assay types, so this is kinda dumb
                pgresult = getPG(pgc,sql,(v_ids,a_ids),LOG)
                ...
                if pgresult !="Error":
                    if len(pgresult) !=0:
                        for x,y,z in pgresult:
                            sql = "delete from wh.pheno_report where vrl_accession=%s and assay_type=%s"
                            execPG(pgc,sql,(x,y),LOG)    # x,y are used to ensure unique report
                            dup_ids.append(str(z))       # z is used to ensure unique data in pheno_rep_row later
                else:
                    LOG.write(gt()+"\twh.pheno_report and wh.pheno_rep_row updates were skipped, there was a problem checking for existing wh.pheno_report records\n")
                    return 'bad'

                # Insert new records into wh.pheno_report
                ##########################################
...
##################################################
def updateGTRuleResultRows(pgc,LOG):
...
def main():
	...
	log_pth = "/opt/bin/logs/dev_log.txt"												# Use for testing on Bali
	buffer_size = 0
	...
	if status == 'ok':
		...
		checkStatus(updateGTRuleResultRows(pgc,LOG))  # connect to MySQL in the method, to avoid connection timeout from upstream delays
