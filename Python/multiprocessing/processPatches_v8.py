...
doMLR<-function(myData, patch) 
{
  ...
  if (isSig == 1) 
  {
    ...
    # return as a vector
    sigVals = c( MLRCoeff[sigPVals==min(sigPVals)], 
...
## Print column title
print(paste (c("PatchFile", "Patch.30CHARs", "Nr-of-Models", "MLR", "MLR.Site", "LogR", "LogR.Site", "SVM-Acc", "SVM-maxSS", "FET", "FET.Site"), collapse=", ") )
...
for(filei in filelist) # loop thru all files
{
  ...
  if (mySVM!=0) { nrModels = nrModels + 1 }
