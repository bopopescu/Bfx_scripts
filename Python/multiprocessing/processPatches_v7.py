...
02.28.2012 Revised to include R code from MinePatch-doAll.R_v7 02.27.12
"""
...
doSVM<-function(myData, patch, maxIC, patchFile) 
{
  ...
  ## or cross-val accuracy > 70%%
  isSig = 0
  if (SVM.Sens > 70 | SVM.Spec > 70 | xValAccuracy > 70) 
  {
    ...
    myTbl = data.frame(SVM.Site, xValAccuracy, SVM.Sens, SVM.Spec, SS, SR, RS, RR)
    print(myTbl)
...
doMLR<-function(myData, patch) 
{
  ...
  #isSig <- ifelse( max(ifelse( sigPVals<0.1, 1, 0), na.rm=FALSE) == 1, 1, 0)
...
for(filei in filelist) # loop thru all files
{
  ...
  {
    ### make one string from patch sites -> then print
    print(paste(patchSites, collapse="-") )
