...
doSVM<-function(myData, patch, maxIC, patchFile) 
{
  ...
  isSig = 0
  if ((SVM.Sens > 70 & SVM.Spec > 30)  | (SVM.Sens > 30 & SVM.Spec > 70)) 
  {   
     isSig = max(SVM.Sens , SVM.Spec)
  }
  return(isSig)
} ########## END of SVM 
...
## Print column title
print(paste (c("PatchFile", "Patch.30CHARs", "Nr-of-Models", "MLR", "MLR.Site", "LogR", "LogR.Site", "SVM", "FET", "FET.Site"), collapse=", ") )
...
for(filei in filelist) # loop thru all files
{
  ...
  if (nrModels > 0) 
  {  
    ### make one string from patch sites -> then trim to 30 chars
    sigPatch = strtrim(paste(patchSites, collapse="-"), 30)
