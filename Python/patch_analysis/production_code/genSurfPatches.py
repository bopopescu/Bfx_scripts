...
###############################################
def processFiles(pdbfile,datafile,size,dpx_cutoff):
    print "Processing for patches that are ",size," angstroms in diameter and less than ",dpx_cutoff," deep"
  ...
  #  print "asa_data2 keys= ",sorted(asa_data2.keys())
    ...
    pnum = 1
    for i in range(0,len(residues)):  # i is length of structure, therfore also = to residue position
        #
        # Add check here that the current residue meets "surface" critera
        #
        # 
        if (float(asa_data2[i]['rsa']) > 0.2) and (float(asa_data2[i]['dpx']) <= dpx_cutoff): # 0, 1.5 or 2.5 Angstrom depths are options
            ...
            working_dist = []                                                                           
        #    print "----------------- Examining ",working_readable                                       
            for j in range(0,len(residues)):                                                            
                if i != j :
                   # print "i= ",i," j= ",j," for residue ",aa_lookup[residues[j].resname],residues[j].id[1]
                    if current_residue_ca - residues[j]['CA'] <= int(size):
                    #    print "residue ",residues[j].id[1],' distance:',str(current_residue_ca - residues[j]['CA']),' rsa:'#,str(asa_data2[j]['rsa']),' DPX: ',asa_data2[j]['dpx']
                        if (float(asa_data2[j]['rsa']) > 0.2) and (float(asa_data2[j]['dpx']) <= dpx_cutoff):  # 0, 1.5 or 2.5 Angstrom depths are options
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"p:d:s:h:a:",["pdb_filename,data_filename,patch_size,help,dpx"])
    ...
    for opt, arg in opts:
        ...
        elif opt in ("-a","--dpx"):  dpx_cutoff = Decimal(arg)
   
    if pdbfile !='' and datafile !='':
        processFiles(pdbfile,datafile,size,dpx_cutoff)
    else:
        usage()
        sys.exit()
