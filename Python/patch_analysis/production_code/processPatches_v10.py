...
02.28.2012 Revised to include R code from MinePatch-doAll-SipleOut.R_v2 02.27.12
"""
...
# Mojgan Haddad - 2-1-2012
# 2-13-2012 release: 
#  1) flipped 0/1 categories to be Sensitive/Resistant so that all models are consistent in the directionality of the coefficients.
#  2) also modified the SVM to only rely on cross validation in the model. Now it would apply the trained model on the whole set.
#  3) ===> SIMPLE output
# 4-2012:
#   1) Added number of significant sites in the patch per MLR
# 6/6/2012:
#   1) re-ordered MLR output for NrSites to go first
#   2) Fixed the nrModels count
##################################################################
...
## Print column title
print(paste (c("PatchFile", "Patch.30CHARs", "Nr-of-Models", "Nr-MLR-SigSites", "MLR.Site", "MLR", "MLR.p", "LogR", "LogR.Site", "SVM-Acc", "SVM-maxSS", "FET", "FET.Site"), collapse=", ") )
...
for(filei in filelist) # loop thru all files
{
  ...
  if (nrModels > 0) 
  {
    ...
    ### make one string from patch sites -> then trim to 50 chars
    #sigPatch = strtrim(paste(patchSites, collapse="-"), 50)
    sigPatch = paste(patchSites, collapse="-")
    
    toPrint = c(filei, sigPatch, nrModels, myMLR, LogR, mySVM, myFET)
